...
export default function PrivacyPolicyPage() {
  const [records, setRecords] = useState<PrivacyPolicyRecord[]>([]);
  ...
  // Load all privacy policy versions
  useEffect(() => {
    (async () => {
      try {
        const data = await get<PrivacyPolicyRecord[]>('/privacy/get');
        setRecords(data);
        if (data.length) {
          const latest = data[0];
          setSelectedId(latest.privacyId);
  ...
  const handleSelect = (e: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    setSelectedId(id);
    const rec = records.find(r => r.privacyId === id);
    if (rec) {
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    try {
      await post<void>('/privacy/updateById', {
        privacyId: selectedId,
        content,
        effectiveDate,
      });
      await Swal.fire({
  ...
  return (
    <div className="p-6 max-w-3xl mx-auto bg-white shadow-md rounded-lg">
      <h1 className="text-3xl font-semibold mb-6">Manage Privacy Policy</h1>
      ...
      <div className="mb-6">
        ...
        >
          {records.map(r => (
            <option key={r.privacyId} value={r.privacyId}>
