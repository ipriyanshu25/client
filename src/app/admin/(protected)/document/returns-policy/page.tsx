...
export default function ReturnsPolicyPage() {
  const [records, setRecords] = useState<ReturnsPolicyRecord[]>([]);
  ...
  // Load all returns policy versions
  useEffect(() => {
    (async () => {
      try {
        const data = await get<ReturnsPolicyRecord[]>('/return/get');
        setRecords(data);
        if (data.length) {
          const latest = data[0];
          setSelectedId(latest.returnId);
        ...
        Swal.fire({
          icon: 'error',
          title: 'Load Failed',
          text: 'Unable to load Returns Policy.',
          timer: 1500,
  ...
  const handleSelect = (e: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    setSelectedId(id);
    const rec = records.find(r => r.returnId === id);
    if (rec) {
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    try {
      await post<void>('/return/updateById', {
        returnId: selectedId,
        content,
        effectiveDate,
      });
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center h-64">
        <p className="text-gray-500">Loading Returns Policy...</p>
      </div>
    );
  }
  ...
  return (
    <div className="p-6 max-w-3xl mx-auto bg-white shadow-md rounded-lg">
      <h1 className="text-3xl font-semibold mb-6">Manage Returns Policy</h1>
