...
export default function ShippingPolicyPage() {
  ...
  useEffect(() => {
    (async () => {
      try {
        const data = await get<ShippingRecord[]>('/shipping/get');
        setRecords(data);
        if (data.length) {
          const latest = data[0];
          setSelectedId(latest.shippingId);
        ...
        Swal.fire({
          icon: 'error',
          title: 'Load Failed',
          text: 'Unable to load Shipping & Delivery Policy.',
          timer: 1500,
  ...
  const handleSelect = (e: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    setSelectedId(id);
    const rec = records.find(r => r.shippingId === id);
    if (rec) {
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    try {
      await post<void>('/shipping/updateById', {
  ...
  return (
    <div className="p-6 max-w-3xl mx-auto bg-white shadow-md rounded-lg">
      <h1 className="text-3xl font-semibold mb-6">Manage Shipping & Delivery Policy</h1>
      ...
      <div className="mb-6">
        ...
        >
          {records.map(r => (
            <option key={r.shippingId} value={r.shippingId}>
